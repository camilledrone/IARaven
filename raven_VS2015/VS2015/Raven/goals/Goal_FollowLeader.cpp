...
void Goal_FollowLeader::Activate()
{
    ...
    position = m_pOwner->GetTeam()->Leader()->Pos() - position;
    ...
    if (m_pOwner->GetPathPlanner()->RequestPathToPosition(m_pOwner->GetTeam()->Leader()->Pos()))
    {
        AddSubgoal(new Goal_SeekToPosition(m_pOwner, position));
    }
}
...
//---------------------------- HandleMessage ----------------------------------
//-----------------------------------------------------------------------------
bool Goal_FollowLeader::HandleMessage(const Telegram& msg)
{
...
void Goal_FollowLeader::Render()
{
    ...
    gdi->BlackPen();
    gdi->BlueBrush();
    gdi->Circle(m_pOwner->GetTeam()->Leader()->Pos(), 6);
    gdi->RedBrush();
    gdi->RedPen();
    gdi->Circle(m_pOwner->GetTeam()->Leader()->Pos(), 4);
    gdi->YellowBrush();
    gdi->YellowPen();
    gdi->Circle(m_pOwner->GetTeam()->Leader()->Pos(), 2);
}
